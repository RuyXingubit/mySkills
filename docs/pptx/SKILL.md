...
- **Templates**: Duplicate and re-arrange template slides using `docs/pptx/scripts/rearrange.py`.
...
- **Workflow**: 
    1. Unpack: `docs/pptx/ooxml/scripts/unpack.py`
    2. Edit XML (`ppt/slides/slideN.xml`).
    3. Validate: `docs/pptx/ooxml/scripts/validate.py`
    4. Pack: `docs/pptx/ooxml/scripts/pack.py`

### 3. Analysis and Text Extraction
- **Markdown Conversion**: Use `markitdown` for simple text extraction.
- **Inventorying**: Extract full shape and text inventories using `docs/pptx/scripts/inventory.py`.
- **Thumbnails**: Generate visual grids for layout analysis using `docs/pptx/scripts/thumbnail.py`.

## Technical Assets
- **Scripts**: Automation scripts located in `docs/pptx/scripts/` and `docs/pptx/ooxml/scripts/`.
- **OOXML Schemas**: Support files located in `docs/pptx/ooxml/schemas/`.
